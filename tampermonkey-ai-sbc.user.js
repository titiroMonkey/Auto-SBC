// ==UserScript==
// DEPRECATED: This Tampermonkey script is no longer the primary deployment method.
// The frontend is now deployed using the persistent injector or console injection methods.
// See README.md for current deployment instructions.
// ==UserScript==
// @name         EAFC 26 Auto SBC
// @namespace    http://tampermonkey.net/
// @version      26.1.12
// @description  automatically solve EAFC 26 SBCs using the currently available players in the club with the minimum cost [DEPRECATED]
// @author       TitiroMonkey
                ...
                if (this._canAddItems()) {
                  ...
                  (this._store.withTxn(function () {
                    if (!(r = i._findAndSelectChoiceByValue(n, !0))) {
                      ...
                      (i._addChoice(
                        G(n, !1, i.config.allowHtmlUserInput),
                        !0,
                        !0,
                      ),
                        (r = !0));
...
`;
  ...
  let dealWithUnassigned = async () => {
    try {
      ...
      const storageToTeam = storage.filter(
        (item) =>
          !players.map((p) => p.definitionId).includes(item.definitionId),
      );
      ...
      const toStorage = ulist
        .filter(
          (item) =>
            item.isStorable() && !item.isMovable() && !item.isTradeable(),
        )
  ...
  const futHomeOverride = async () => {
    ...
    UTHomeHubView.prototype.init = async function () {
      ...
      await fetchPlayerPrices(
        sbcs.sets
          .filter((s) => s.awards[0]?.item)
          .map((s) => s.awards[0]?.item),
      );
      ...
      if (getSettings(sbcId, sbcData.challengeId, "useConcepts")) {
        if (conceptPlayersCollected) {
          players = players.concat(
            conceptPlayers?.filter(
              (f) => !PriceItems[f.definitionId].isExtinct,
            ),
          );
  ...
  const tryQuickBuy = async (context = {}, item, options = {}) => {
    ...
    try {
      ...
      if (bidAttempt && typeof bidAttempt.observe === "function") {
        return await new Promise((resolve) => {
          bidAttempt.observe(context, async (_obs, response) => {
            ...
            notify(
              success
                ? `Quick buy success at ${priceLabel}`
                : "Quick buy failed",
              success
                ? UINotificationType.POSITIVE
                : UINotificationType.NEGATIVE,
      ...
      notify(
        `Quick buy attempted at ${priceLabel}`,
        UINotificationType.POSITIVE,
      );
      return {
        success: true,
  ...
  let PriceItem = function (items) {
    ...
    for (let key in items) {
      ...
      if (
        items[key]["eaId"] &&
        items[key]["eaId"].toString().includes("_CBR")
      ) {
      ...
      request.onsuccess = function (event) {
        ...
        transaction.onerror = function (error) {
          ...
          localStorage.setItem(
            PRICE_ITEMS_KEY,
            JSON.stringify(cachedPriceItems),
          );
        };
      };
  ...
  let fetchLivePlayerPrice = async (player) => {
    ...
    const ensureResults = async (cap) => {
      ...
      if (evalResult.count === 0) {
        ...
        if (evalResult.count === 0) {
          ...
          if (fallback.count < MAX_RESULTS)
            return { cap: null, eval: fallback };
    ...
    if (!upperCap || !Number.isFinite(upperCap)) {
      ...
      if (upperEval.count === 0) {
        ...
        if (upperEval.count < MAX_RESULTS)
          return finishSuccess("stepped-upper");
      }
            ...
            const resetButton = createButton(
              ...
              () => {
                ...
                if (
                  ...
                  if (
                    Object.keys(settings["sbcSettings"][sbcId]).length === 0
                  ) {
  ...
  const createLogOverlayToggle = () => {
    ...
    toggleButton.textContent = getSettings(0, 0, "showLogOverlay")
      ? "ON"
      : "OFF";
    ...
    toggleButton.addEventListener("click", () => {
      ...
      toggleButton.style.backgroundColor = !currentState
        ? "#00ff00"
        : "#ff0000";
      updateLogOverlay();
    });
  ...
  const migrateMaxRatingSettings = () => {
    ...
    Object.entries(sbcSettings).forEach(([sbcId, challenges]) => {
      ...
      Object.entries(challenges).forEach(([challengeId, config]) => {
        ...
        console.log(
          `[SBC] migrateMaxRatingSettings: migrating sbcId=${sbcId}, challengeId=${challengeId}`,
        );
        ...
        if (Number.isFinite(maxRating)) {
          ...
          needsSave = true;
          console.log(
            `[SBC] migrateMaxRatingSettings: updated ratingRange=${currentRange}`,
          );
        }
    ...
    if (e.key === "q" && !e.repeat) {
      ...
      if (nextPack) {
        ...
        const tierCounts = {
          ...
          between82and88: playerRatings.filter((r) => r >= 82 && r <= 88)
            .length,
